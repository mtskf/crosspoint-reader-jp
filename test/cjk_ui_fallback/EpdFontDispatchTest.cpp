...
EpdFontData makeFakeData() {
  ...
  d.bitmap = kBitmap; d.glyph = &kGlyphA;
  d.intervals = kIntervals; d.intervalCount = 1;  // must be >0 so getGlyph reaches the CJK branch
  d.advanceY = 14; d.ascender = 12; d.descender = 2; d.is2Bit = false;
  return d;
}
}  // namespace
...
TEST(EpdFontDispatch, FlagOnCjkSynthesized) {
  EpdFontData d = makeFakeData();
  EpdFont f(&d); f.enableCjkUiFallback();
